...
const STATION_LINES_URL = '../data/master/station_lines_tokyo.json';
const LINES_MASTER_URL  = '../data/master/lines_tokyo_master.json';
...
async function loadStationLines() {
  ...
  const res = await fetch(STATION_LINES_URL);
  if (!res.ok) throw new Error(`Failed to load station_lines_tokyo.json: HTTP ${res.status}`);
  _stationLinesCache = await res.json();
  return _stationLinesCache;
}

async function loadLinesMaster() {
  if (_linesMasterCache) return _linesMasterCache;
  const res = await fetch(LINES_MASTER_URL);
  if (!res.ok) throw new Error(`Failed to load lines_tokyo_master.json: HTTP ${res.status}`);
...
async function computeRouteScore(playerStations, options = {}) {
  ...
  const stationLines = options.stationLines || await loadStationLines();
  const linesMaster  = options.linesMaster  || await loadLinesMaster();
